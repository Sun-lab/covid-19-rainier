...
for(i in 1:nrow(d10)){
  ...
  gcn = ggplot(us, aes(x=day, y=log10(new.case + 1), color=month)) +
    geom_point()  + geom_smooth(method=loess)
  
  gdn = ggplot(us, aes(x=day, y=log10(new.death + 1), color=month)) +
    geom_point()  + geom_smooth(method=loess)
  
  figure = ggarrange(gcn, gdn, ncol = 2, nrow = 1)
  
  str1 = d10$Region[i]
  # str2 = "data resource: https://github.com/CSSEGISandData/COVID-19, "
  # str2 = paste0(str2, "day 1 is Jan 22nd, 2020")
  str2 = "day 1 is Jan 22nd, 2020"
  
  figure = annotate_figure(figure,
                  top = text_grob(str1, color = "brown", face = "bold"),
                  bottom = text_grob(str2, color = "blue", hjust = 1, 
                                     x = 1, face = "italic"))
  print(figure)
...
state10 = states[which(states$date==current.date),]
state10 = state10[order(state10$deaths, decreasing = TRUE)[1:10],]
print(state10)
```
...
for(k in 1:nrow(state10)){
  ...
  gcn = ggplot(df1, aes(x=day, y=log10(new.case + 1), color=month)) +
    geom_point()  + geom_smooth(method=loess)
  
  gdn = ggplot(df1, aes(x=day, y=log10(new.death + 1), color=month)) +
    geom_point()  + geom_smooth(method=loess)
  
  figure = ggarrange(gcn, gdn, ncol = 2, nrow = 1)
  
  str1 = sprintf("%s", st1)
  # str2 = "https://github.com/nytimes/covid-19-data"
  # str2 = paste(str1, str2)
  
  str3 = sprintf("day 1 is %s", df1$date[1])
...
s10 = s10[which(s10$state %in% state10$state),]
...
```{r echo = FALSE, warning = FALSE, message = FALSE}
county10 = counties[which(counties$date==current.date),]
county10 = county10[order(county10$deaths, decreasing = TRUE)[1:10],]
print(county10)
```
...
for(k in 1:nrow(county10)){
  
  ct1 = county10$county[k]
  st1 = county10$state[k]
...
rate = ggplot(states[w2kp,], aes(x=day, y=positive.rate, 
                                 shape=Month, color=state)) +
  geom_point()  + geom_smooth(method=loess) + 
...
str1 = sprintf("cumulative positive rate on %s: ", current.date)
